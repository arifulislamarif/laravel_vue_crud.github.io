    ...
    <div class="col-md-12">
        <div class="card">
            ...
            <div class="card-tools" style="position:absolute;right:1rem;top: .5rem;">
              <button @click="create" class="btn btn-info" type="button">Add <i class="fa fa-plus"></i></button>
              <button @click="reload" class="btn btn-primary" type="button">Reload <i class="fa fa-refresh"></i></button>
            </div>
            </div>
                    ...
                    <tbody>
                      <tr v-show="customers.length" v-for="(customer,index) in customers" :key="customer.id">
                        ...
                        <td class="text-center">
                          ...
                          <button @click="destroy(customer)" type="button" class="btn btn-danger btn-sm">
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="showModalLabel">Show Customer</h5>
          ...
          <div class="modal-body">
             <div class="form-group">
              <label>Name</label>
              <input disabled  :value="form.name" class="form-control">
            </div>
             <div class="form-group">
              <label>Email</label>
              <input disabled  :value="form.email" class="form-control">
            </div>
             <div class="form-group">
              <label>Phone</label>
              <input disabled  :value="form.phone" class="form-control">
            </div>
             <div class="form-group">
              <label>Total</label>
              <input  disabled :value="form.total" class="form-control">
            </div>
             <div class="form-group">
              <label>Address</label>
              <input disabled  :value="form.address" class="form-control">
            </div>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
...
export default {
  ...
  store(){
    ...
    .then(response => {
      ...
      if(this.form.successful){
        this.$Progress.finish()
          this.$snotify.success('Customer Data Successfully Inserted!','success')
      }else{
          this.$Progress.fail()
  ...
  update(){
    ...
    .then(response => {
      ...
      if(this.form.successful){
        this.$Progress.finish()
          this.$snotify.success('Customer Data Successfully Updated!','success')
      }else{
          this.$Progress.fail()
          this.$snotify.error('Something went wrong try again later!','error')
      }
